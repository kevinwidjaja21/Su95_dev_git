(function (global, factory) {
  ...
  class NXDataStore {
    ...
    static get(key, defaultVal) {
      const val = GetStoredData("SU95_".concat(key)); // GetStoredData returns null on error, or empty string for keys that don't exist (why isn't that an error??)
    ...
    static set(key, val) {
      SetStoredData("SU95_".concat(key), val);
  ...
  const fetchWithTimeout = function (resource, options) {
    let timeout = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 200;
