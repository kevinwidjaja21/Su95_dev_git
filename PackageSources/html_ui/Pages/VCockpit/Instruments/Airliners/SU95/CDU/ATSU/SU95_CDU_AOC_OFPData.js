...
class CDUAocOfpData {
    ...
    static ShowPage2(mcdu) {
        ...
        if (currentZfwcg) {
            const cgColor = currentZfwcg >= 12.5 && currentZfwcg <= 30 ? 'cyan' : 'red';
...
async function loadBaggagePayload(mcdu, updateView) {
    const FORWARD_BAGGAGE_INDEX = 4; // Value from flight_model.cfg
    const REAR_BAGGAGE_INDEX = 5; // Value from flight_model.cfg
...
async function loadFuel(mcdu, updateView) {
    ...
    const outerTankCapacity = 110; // Left and Right // Value from flight_model.cfg
    const innerTankCapacity = 1260; // Left and Right // Value from flight_model.cfg
    const centerTankCapacity = 1475; // Center // Value from flight_model.cfg
