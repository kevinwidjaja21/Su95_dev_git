...
class CDUAocOfpData {
    ...
    static ShowPage2(mcdu) {
        ...
        if (currentZfwcg) {
            const cgColor = currentZfwcg >= 12.5 && currentZfwcg <= 3	0 ? 'cyan' : 'red'; // Changed for SU95
        ...
        mcdu.onRightInput[1] = (value) => {
            ...
            const minAllowableZfwcg = 12.5; // changed to SU95
            const maxAllowableZfwcg = 30;
...
async function loadBaggagePayload(mcdu, updateView) {
    const FORWARD_BAGGAGE_INDEX = 4; // Value from flight_model.cfg SU95
    const REAR_BAGGAGE_INDEX = 5; // Value from flight_model.cfg SU95
...
async function loadFuel(mcdu, updateView) {
    ...
    const outerTankCapacity = 0; // Left and Right // Value from flight_model.cfg
    const innerTankCapacity = 1336; // Left and Right // Value from flight_model.cfg
    const centerTankCapacity = 440; // Center // Value from flight_model.cfg
...
function getDistributedBaggagePayload(payload = 0, percentMacTarget = 30) {
    ...
    const EMPTY_AIRCRAFT_WEIGHT = 53460 * 0.453592; // Value from flight_model.cfg to kgs
    const EMPTY_AIRCRAFT_POSITION = -3.56; // Value from flight_model.cfg
    const FORWARD_BAGGAGE_POSITION = 10.5; // Value from flight_model.cfg
    const REAR_BAGGAGE_POSITION = -24; // Value from flight_model.cfg
