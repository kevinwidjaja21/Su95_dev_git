...
export function getTemplatePlugin({ name, config, imports = [], isInstrument }) {
    if (isInstrument) {
        return instrumentTemplate({
            ...
            config,
            imports,
            outputDir: join(Directories.root, 'PackageSources/html_ui/Pages/VCockpit/Instruments/A32NX_SU95'),
        });
    }
    return ecamPageTemplate({
        name,
        outputDir: join(Directories.root, 'PackageSources/html_ui/Pages/VCockpit/Instruments/A32NX_SU95/EcamPages'),
    });
}
