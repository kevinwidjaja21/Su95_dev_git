...
export function getInstrumentsIgniterTasks() {
    ...
    return [
        ...baseInstruments.map(({ name }) => new ExecTask(
            ...
            [join('src/instruments/src', name), join('PackageSources/html_ui/Pages/VCockpit/Instruments/A32NX_SU95', name)],
        )),
        ...ecamPages.map(({ name, path }) => new ExecTask(
            name,
            `node src/instruments/buildSrc/igniter/worker.mjs ${name}`,
            [join('src/instruments/src', path), join('PackageSources/html_ui/Pages/VCockpit/Instruments/A32NX_SU95/EcamPages', name)],
        )),
    ];
}
